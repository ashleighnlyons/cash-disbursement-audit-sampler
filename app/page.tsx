...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero"><div className="eyebrow">AUDIT OPERATIONS</div><h1>Cash Disbursement<br/><em>Audit Sampler</em></h1><p>Turn a paid-expense report into a documented, reproducible audit sample without uploading it anywhere.</p></section>
    <section className="workspace">
      ...
      <div className="panelGrid">
        <section className="uploadCard">
          ...
          <div className="methodPicker" role="radiogroup" aria-label="Accounting system">
          ...
          <div className="sectionLabel">Choose {sourceSystem === "appfolio" ? "AppFolio" : "Yardi"} report</div>
          <button type="button" className={`dropzone ${file ? "hasFile" : ""}`} onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); choose(event.dataTransfer.files[0]); }}><input ref={inputRef} type="file" accept=".xlsx" hidden onChange={(event) => choose(event.target.files?.[0] || null)}/><span className="uploadIcon">↥</span>{file ? <><strong>{file.name}</strong><small>{(file.size / 1024).toFixed(0)} KB · Ready to process</small></> : <><strong>Drop your {sourceSystem === "appfolio" ? "AppFolio" : "Yardi"} report here</strong><small>or click to browse · .xlsx only</small></>}</button>
        ...
        <aside className="rulesCard">
          ...
          <div className="rule"><span className="ruleIcon">{sourceSystem === "appfolio" ? "A" : "Y"}</span><div><b>{sourceSystem === "appfolio" ? "AppFolio payment" : "Yardi Check Control"}</b><small>{sourceSystem === "appfolio" ? "Payment header and its GL lines stay together" : "Expense lines are grouped into one payment"}</small></div></div>
